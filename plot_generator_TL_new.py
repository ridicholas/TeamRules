        ...
        for cost in costs:
            ...
            if tr_optimizer:
                ...
                for temp_cost in costs:
                    ...
                    temp_data = pd.read_pickle(path + 'val_cost_{}_'.format(temp_cost) + team + '_tr_filtered_run{}.pkl'.format(i)).sort_values(by=['objective', 'contradicts']).reset_index()
                    temp_obj = temp_data.loc[0,'val_error'] + float(cost)*temp_data.loc[0,'contradicts']/(datasets[i].shape[0])
                    if temp_obj < best_val_obj:
                        best_val_obj = temp_obj
                        opt_cost_dict[cost] = temp_cost
                        #tr_conf = temp_data.loc[0, 'mental_conf']
