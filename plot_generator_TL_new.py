...
for whichType in types:
    ...
    for whichTeam in range(len(settings)):
        ...
        for cost in costs:
            ...
            costFrame.loc[cost, 'TR_Contradictions'] = mean(np.array(TRContradicts)*float(cost))
            costFrame.loc[cost, 'TR_Contradictions_std'] = stdev(np.array(TRContradicts)*float(cost))/math.sqrt(numRuns)
            costFrame.loc[cost, 'HyRS_Contradictions'] = mean(np.array(HyRSContradicts)*float(cost))
            costFrame.loc[cost, 'HyRS_Contradictions_std'] = stdev(np.array(HyRSContradicts)*float(cost))/math.sqrt(numRuns)
            costFrame.loc[cost, 'R_HyRS_Contradictions'] = mean(np.array(R_HyRSContradicts)*float(cost))
            costFrame.loc[cost, 'R_HyRS_Contradictions_std'] = stdev(np.array(R_HyRSContradicts)*float(cost))/math.sqrt(numRuns)
            ...
            costFrame.loc[cost, 'BRS_Contradictions'] = mean(np.array(BRSContradicts)*float(cost))
            costFrame.loc[cost, 'BRS_Contradictions_std'] = stdev(np.array(BRSContradicts)*float(cost))/math.sqrt(numRuns)
        ...
        l = list(zip(R_HyRS_loss, R_HyRS_con))
        z = list(zip(*[i for n, i in enumerate(l) if i not in l[:n]]))
        R_HyRS_loss = np.array(z[0])
        R_HyRS_con = np.array(z[1])
        ...
        if whichType == 'TL':
            ...
            plt.plot(costFrame['Costs'], costFrame['HyRS_Objective'], marker = 'v', c=color_dict['HYRS'], label = 'TR-No(ADB, OrgVal)', markersize=1.8, linewidth=0.9)
            ...
            plt.plot(costFrame['HyRS_Contradictions'], costFrame['HyRSTeamLoss'], marker = 'v', markersize=2, c=color_dict['HYRS'], label = 'TR-No(ADB, OrgVal)', linewidth=0.9)
            plt.plot(costFrame['R_HyRS_Contradictions'], costFrame['R_HyRSTeamLoss'], marker = 'v', markersize=2, c=color_dict['HYRSRecon'], label = 'TR-No(ADB)', linewidth=0.9)
            
            
            cost = '0'

            #for cost in costFrame.Costs:
            #    plt.fill_between(np.linspace(costFrame.loc[cost, 'R_HyRS_Contradictions']-costFrame.loc[cost, 'R_HyRS_Contradictions_std'], 
            #                            costFrame.loc[cost, 'R_HyRS_Contradictions']+costFrame.loc[cost, 'R_HyRS_Contradictions_std'], 
            ...
            plt.plot(costFrame['BRS_Contradictions'], costFrame['BRSTeamLoss'], marker = 's', markersize=2, c=color_dict['BRS'], label = 'Task-Only (Current Practice)', linewidth=0.9)
            ...
            plt.plot(costFrame['TR_Contradictions'], costFrame['TeamRulesTeamLoss'], marker='.', markersize=2, c=color_dict['TR'], label='TR', linewidth=0.9)
            ...
            plt.xlabel('Cost Incurred', fontsize=12)
