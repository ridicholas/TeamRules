...
for i in range(0, numRuns):
    ...
    #team_infos.append(pd.read_pickle(path + 'team_info_run{}.pkl'.format(i)))
    for team in teams:
        for cost in costs:
...
for whichTeam in range(len(settings)):
    ...
    for cost in costs:
        ...
        for run in range(numRuns):
            ...
            #replace 'error_rate_acceptRegion' with 'test_error' for TL

            TeamRulesLoss.append(tr_results_filtered[team][cost][run].loc[0,'error_rate_acceptRegion'])
            ...
            HyRSLoss.append(hyrs_results_filtered[team][cost][run].loc[0,'error_rate_acceptRegion'])
            ...
            TR_Objectives.append((TeamRulesLoss[-1]*(datasets[run]['{}TestConf'.format(team)] <= 0.5).sum() + float(cost)*TRContradicts[-1])/(datasets[run].shape[0]))
            HyRS_Objectives.append((HyRSLoss[-1]*(datasets[run]['{}TestConf'.format(team)] <= 0.5).sum() + float(cost)*HyRSContradicts[-1])/(datasets[run].shape[0]))
    ...
    #costFrame['TR_Objective'] = (costFrame['TeamRulesTeamLoss']*(datasets[run]['{}TestConf'.format(team)] <= 0.5).sum() + (costFrame['Costs'].astype(float))*costFrame['TR_Contradictions'])/(datasets[run].shape[0])
    #costFrame['HyRS_Objective'] = (costFrame['HyRSTeamLoss']*(datasets[run]['{}TestConf'.format(team)] <= 0.5).sum() + (costFrame['Costs'].astype(float))*costFrame['HyRS_Contradictions'])/(datasets[run].shape[0])
    ...
    for run in range(numRuns):
        for cost in costs:
            cost = str(cost)
            TR_loss.append(tr_results_filtered[team][cost][run].loc[0,'error_rate_acceptRegion'])
            TR_con.append(tr_results_filtered[team][cost][run].loc[0,'contradictions'])
            HyRS_loss.append(hyrs_results_filtered[team][cost][run].loc[0,'error_rate_acceptRegion'])
    ...
    for row in ax:
        if i==0:
            ...
            row.set_ylabel('TDLa', fontsize=14)
            row.tick_params(labelrotation=45, labelsize=10)
            #row.set_title('{} Setting'.format(setting), fontsize=15)
